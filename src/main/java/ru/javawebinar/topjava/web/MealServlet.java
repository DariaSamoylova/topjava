        ...
        switch (action == null ? "all" : action) {
            ...
            case "update":
                final Meal meal = "create".equals(action) ?
                        mealRestController
                                .create(new Meal(LocalDateTime.now().truncatedTo(ChronoUnit.MINUTES), "", 1000, SecurityUtil.authUserId())) :
            ...
            default:
                ...
                if (request.getParameter("filter") != null) {
                    request.setAttribute("meals", mealRestController.
                            getAllToFiltered(
                                    LocalDate.parse(request.getParameter("date1")),
                                    LocalDate.parse(request.getParameter("date2")),
                                    LocalTime.parse(request.getParameter("time1")),
                                    LocalTime.parse(request.getParameter("time2"))));
                } else {
